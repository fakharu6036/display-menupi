...
if (portNum === 3306 || portNum === 5432 || portNum === 27017) {
    ...
    console.error('');
    console.error('💡 DETECTED: Railway CLI is linked to MySQL service instead of Web service!');
    console.error('');
    console.error('🔧 IMMEDIATE FIX REQUIRED:');
    console.error('   1. Railway Dashboard → https://railway.app/dashboard');
    console.error('   2. Go to your project → Find your WEB SERVICE (not MySQL)');
    console.error('   3. Copy the service ID or name');
    console.error('   4. Run: railway service link [web-service-id-or-name]');
    console.error('   5. Verify: railway service status (should show web service, not MySQL)');
    console.error('   6. Redeploy: railway up --detach');
    console.error('');
